...
1. Add release notes as files in `.changelog-unreleased/` (use filenames without whitespace).
2. Create a temporary release branch and run:
    ```
    ./util/bump_version.sh --patch  
    # or --minor, --major, or an explicit version such as 0.8.0
    ```
   This collects the notes, updates the versions, removes the note files, and commits the changes.

3. Verify the generated changelog against the commit log to ensure nothing was missed.

4. Open and merge a release PR. The title should be `chore: release vX.Y.Z.` (with your new version number).

5. Tag the merged release commit and push the tag:
    ```
...
7. Confirm that the new release showed up at the [releases page](https://github.com/verus-lang/verusfmt/releases) and the new version appears on [crates.io](https://crates.io/crates/verusfmt). If it does not, then likely some update is needed to the auto-release process.  In such a case, the tagged `vX.Y.Z` should be considered "used", and the next attempt should be `vX.Y.{Z+1}`.
