...
use crate::definitions_t::{
    aligned, axiom_max_phyaddr_width_facts, between, new_seq, x86_arch_spec, MAX_BASE,
};
use crate::definitions_t::{
    bit, bitmask_inc, x86_arch_exec, Flags, MemRegion, MemRegionExec, PageTableEntry,
    PageTableEntryExec, L1_ENTRY_SIZE, L2_ENTRY_SIZE, L3_ENTRY_SIZE, MAX_PHYADDR,
...
impl PageDirectoryEntry {
    ...
    pub proof fn lemma_addr_mask_when_hp_pat_is_zero(self)
        requires
            self.hp_pat_is_zero() && self.all_mb0_bits_are_zero() && self@ is Page,
        ensures
    ...
    pub fn new_entry(
        ...
        ensures
            r.all_mb0_bits_are_zero(),
            if is_page {
                r@ is Page && r@.get_Page_addr() == address
            } else {
                r@ is Directory && r@.get_Directory_addr() == address
            },
            r.hp_pat_is_zero(),
            r.layer@ == layer,
...
pub mod PT {
    ...
    pub open spec fn directories_have_flags(
        ...
        forall|i: nat|
            i < X86_NUM_ENTRIES ==> {
                let entry = #[trigger] view_at(mem, pt, layer, ptr, i);
                entry is Directory ==> entry.get_Directory_flag_RW()
            ...
            ({
                ...
                match view_at(mem, pt, layer, ptr, i) {
                    GhostPageDirectoryEntry::Directory { addr: dir_addr, .. } => {
                        ...
                        &&& res.entries[i as int]->Directory_0 == interp_at(
                            mem,
                            pt.entries[i as int].get_Some_0(),
                            (layer + 1) as nat,
                            dir_addr,
                    ...
                    GhostPageDirectoryEntry::Page { addr, .. } => res.entries[i as int] is Page
                        && res.entries[i as int]->Page_0.frame.base == addr,
                ...
                &&& (forall|j: nat|
                    ...
                    init.len() <= j && j < res.len() ==> match view_at(mem, pt, layer, ptr, j) {
                        GhostPageDirectoryEntry::Directory { addr: dir_addr, .. } => {
                            ...
                            &&& res[j as int]->Directory_0 == interp_at(
                                mem,
                        ...
                        GhostPageDirectoryEntry::Page { addr, .. } => res[j as int] is Page
                            && res[j as int]->Page_0.frame.base == addr,
            ...
            if entry.is_dir(layer) {
                ...
                proof {
                    if interp@.resolve(vaddr as nat).is_Ok() {
                        assert(interp@.entries[idx as int]->Page_0 === interp@.resolve(
                            vaddr as nat,
                        ).get_Ok_0().1);
            ...
            inv_at(mem, pt, layer, ptr),
    {
        assert forall|i: nat| #![auto] i < X86_NUM_ENTRIES implies entry_at_spec(
            ...
            ptr,
            i,
        )@ is Empty && entry_at_spec(mem, pt, layer, ptr, i).all_mb0_bits_are_zero() by {
