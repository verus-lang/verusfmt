...
#[cfg(verus_keep_ghost)]
use crate::definitions_t::{aligned, between, overlap};
use crate::definitions_t::{Arch, Flags, MemRegion, PageTableEntry};
#[cfg(verus_keep_ghost)]
...
pub proof fn ambient_lemmas2()
    ensures
        forall|d: Directory, i: nat|
            ...
            d.inv() && i < d.num_entries() && d.entries.index(i as int) is Directory
                ==> d.entries.index(i as int)->Directory_0.inv(),
        ...
        forall|d: Directory| d.inv() ==> (#[trigger] d.interp()).lower == d.base_vaddr,
{
    assert forall|d: Directory, i: nat|
        ...
        d.inv() && i < d.num_entries() && d.entries.index(
            i as int,
        ) is Directory implies d.entries.index(i as int)->Directory_0.inv() by {
...
impl Directory {
    ...
    {
        forall|i: nat|
            (i < self.entries.len() && self.entries[i as int] is Page) ==> (
    ...
    {
        forall|i: nat|
            i < self.entries.len() && self.entries.index(i as int) is Directory ==> {
        ...
        decreases self.arch.layers.len() - self.layer, 0nat,
    {
        if self.well_formed() && self.directories_are_in_next_layer()
            && self.directories_match_arch() {
            forall|i: nat|
                (i < self.entries.len() && #[trigger] (self.entries[i as int] is Directory)) ==> (
    ...
    {
        forall|i: nat|
            i < self.entries.len() && #[trigger] (self.entries.index(i as int) is Page) ==> aligned(
                (#[trigger] self.entries.index(i as int)->Page_0).frame.base,
                self.entry_size(),
            )
    }
