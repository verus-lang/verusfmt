...
impl HostState {
            ...
            }),
    {
        ...
        if !ok {
            ...
            proof {
                ...
                assert forall|i|
                    0 <= i < ios.len()
                        && #[trigger] ios[i].is_Send() implies !ios[i].get_Send_s().msg.is_InvalidMessage() by {
    ...
    pub closed spec fn host_state_common_postconditions(
        ...
        &&& (forall|i: int|
            0 <= i && i < sent_packets.len() ==> (#[trigger] sent_packets[i].msg).is_Message()
                || sent_packets[i].msg.is_Ack())
        &&& abstractify_seq_of_cpackets_to_set_of_sht_packets(sent_packets)
            =~= extract_packets_from_lsht_packets(
            abstractify_outbound_packets_to_seq_of_lsht_packets(sent_packets),
        )
...
pub open spec fn only_sent_marshalable_data(rawlog: Seq<NetEvent>) -> bool {
    forall|i|
        0 <= i < rawlog.len() && rawlog[i].is_Send() ==> #[trigger] is_marshalable_data(rawlog[i])
}
        ...
        }),
{
            ...
            forall|i| 0 <= i < net_events.len() ==> net_events[i] is Send,
    {
        ...
        proof {
            ...
            assert forall|j|
                0 <= j < net_events.len()
                    && net_events[j].is_Send() implies #[trigger] is_marshalable_data(
                net_events[j],
            ) by {
...
pub proof fn set_map_union_auto<A, B>()
    ensures
        forall|s1: Set<A>, s2: Set<A>, f: FnSpec(A) -> B| #[trigger]
            (s1 + s2).map(f) == s1.map(f) + s2.map(f),
{
    assert forall|s1: Set<A>, s2: Set<A>, f: FnSpec(A) -> B| #[trigger]
...
pub proof fn seq_map_values_concat_auto<A, B>()
    ensures
        forall|s1: Seq<A>, s2: Seq<A>, f: FnSpec(A) -> B| #[trigger]
            (s1 + s2).map_values(f) == s1.map_values(f) + s2.map_values(f),
{
    assert forall|s1: Seq<A>, s2: Seq<A>, f: FnSpec(A) -> B| #[trigger]
