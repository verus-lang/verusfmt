...
struct StrictlyOrderedMap<#[verifier(maybe_negative)] K: KeyTrait + VerusClone> {
...
pub struct DelegationMap<#[verifier(maybe_negative)] K: KeyTrait + VerusClone> {
...
pub struct HashMap<#[verifier(strictly_positive)] V> {
...
pub struct LEnvironment<#[verifier(maybe_negative)] IdType, #[verifier::maybe_negative] MessageType> {
    pub time:int,
...
// (If Key has a nondeterministic hash, this ensures will be a lie.)
    ...
    // This thing should be a Verus library
    pub open spec fn predicate_models<T, EF: Fn(T)->bool>(
        exec_fn: EF, spec_fn: FnSpec(T)->bool) -> bool
    {
        &&& forall |t| #![auto] exec_fn.requires((t,))
        &&& forall |t, b| exec_fn.ensures((t,), b) ==> spec_fn(t)==b
    }

    #[verifier(external_body)] // iter is not supported
    pub fn filter<F: Fn(CKey)->bool>(&self, f: F, fs: Ghost<FnSpec(CKey)->bool>) -> (res: Self)
    requires
...
impl EventResults {
    ...
    pub open spec fn well_typed_events(self) -> bool {
        &&& forall |i| 0 <= i < self.recvs.len() ==> self.recvs[i].is_Receive()
        &&& forall |i| 0 <= i < self.clocks.len() ==> self.clocks[i].is_ReadClock() || self.clocks[i].is_TimeoutReceive()
        &&& forall |i| 0 <= i < self.sends.len() ==> self.sends[i].is_Send()
        &&& self.clocks.len() <= 1
    }
        ...
        res@.well_typed_events(),
{
    ...
    assert (forall |i| 0 <= i < res.clocks.len() ==> res.clocks[i].is_ReadClock() || res.clocks[i].is_TimeoutReceive());
...
impl HostState {
        ...
        if !ok {
            ...
            proof {
                assert (abstractify_seq_of_cpackets_to_set_of_sht_packets(packets@) ==
                    ...
                    lemma_if_everything_in_seq_satisfies_filter_then_filter_is_identity(ios, |io: LSHTIo| io.is_Send());
                ...
                assert forall |i| 0 <= i < ios.len() && #[trigger] ios[i].is_Send() implies !ios[i].get_Send_s().msg.is_InvalidMessage() by {
    ...
    requires
        ...
        abstractify_cpacket_to_lsht_packet(cpacket) == abstractify_net_packet_to_lsht_packet(receive_event.get_Receive_r()),
    ensures ({
...
//         &&& pre.host_state_common_preconditions()
        ...
        &&& (forall |i: int| 0 <= i && i < sent_packets.len() ==>
              (#[trigger] sent_packets[i].msg).is_Message() || sent_packets[i].msg.is_Ack())
        &&& abstractify_seq_of_cpackets_to_set_of_sht_packets(sent_packets) =~=
            extract_packets_from_lsht_packets(abstractify_outbound_packets_to_seq_of_lsht_packets(sent_packets))
        &&& self.resend_count < 100000000
    }
...
//                         }
                        proof {
                            ...
                            assert (next_get_request_reply(
                                        old(self)@, self@, pkt.src, pkt.msg.get_Message_seqno(),
                            ...
                            match optional_sm {
                                ...
                                None => {
                                    ...
                                    proof {
                                        ...
                                        assert( next_set_request_complete(old(self)@, self@, pkt.src, pkt.msg.get_Message_seqno(), pkt.msg.get_Message_m(), arbitrary(), arbitrary(), abs_sent_packets, false) );   // exists witness
                                    }
                                    return sent_packets;
                                }
                            }
        ...
        Self::next_ensures(*old(self), *old(netc), *self, *netc, rc),
    {
        ...
        proof {
            ...
            assert forall |i| #![auto] 0 <= i < aios.len() && aios[i].is_Send()
                implies !aios[i].get_Send_s().msg.is_InvalidMessage() by {
            ...
            lemma_if_everything_in_seq_satisfies_filter_then_filter_is_identity(aios, |io: LSHTIo| io.is_Send());
...
pub open spec(checked) fn receive_packet_next(pre: AbstractHostState, post: AbstractHostState, ios: AbstractIos) -> bool {
    ...
    &&& if ios[0].is_TimeoutReceive() {
            ...
            &&& pre.delegation_map.is_complete()
            &&& ios[0].is_Receive()
            &&& forall |i| 1 <= i < ios.len() ==> /*#[trigger]*/ ios[i].is_Send()
        ...
        pre.delegation_map.is_complete(),
{
    ...
    &&& (exists |sm,m,b| next_get_request_reply(pre, post, pkt.src, pkt.msg.get_Message_seqno(), pkt.msg.get_Message_m().get_GetRequest_key(), sm, m, out, b))
}
        ...
        pre.delegation_map.is_complete(),
{
    ...
    &&& exists |sm: SingleMessage<Message>, replym: Message, should_send: bool| next_set_request_complete(pre, post, pkt.src, pkt.msg.get_Message_seqno(), pkt.msg.get_Message_m(), sm, replym, out, should_send)
    &&& post.delegation_map == pre.delegation_map
    &&& post.num_delegations == pre.num_delegations
}
        ...
        pre.delegation_map.is_complete(),
{
    ...
    &&& if pre.constants.host_ids.contains(pkt.src) {
            ...
            &&& post.delegation_map == pre.delegation_map.update(m.get_Delegate_range(), pre.constants.me)
...
{
    &&& pre.delegation_map.is_complete()
    &&& forall |i| 0 <= i < ios.len() ==> ios[i].is_Send()
...
pub open spec(checked) fn no_invalid_sends(ios: AbstractIos) -> bool {
    forall |i| #![auto] 0 <= i < ios.len() && ios[i].is_Send() ==> !ios[i].get_Send_s().msg.is_InvalidMessage()
}
...
impl NetClient {
    ...
    pub fn reset(&mut self)
        ...
        ensures
            ...
            // TODO: surely something needs to be said about history?
    {
    ...
    pub fn receive(&mut self, time_limit_s: i32) -> (result: NetcReceiveResult)
        requires
          // TODO(verus:jonh): start a discussion about demanding old(self) in requires
          old(self).state().is_Receiving()
        ensures
          self.my_end_point() == old(self).my_end_point(),
          match result {
    ...
    pub fn send(&mut self, recipient: &EndPoint, message: &Vec<u8>) -> (result: Result<(), IronfleetIOError> )
        ...
        ensures
            ...
            result.is_Ok() ==> self.history() == old(self).history() + seq![LIoOp::Send{s: LPacket{dst: recipient@, src: self.my_end_point(), msg: message@}}],
    {
...
impl Marshalable for Vec<u8> {
  ...
  {
    self.len() <= usize::MAX &&
    self.len().serialized_size() <= usize::MAX - self.len()
  }
...
impl<T: Marshalable> Marshalable for Vec<T> {
    ...
    // req, ens from trait
  {
    ...
    let accf: Ghost<FnSpec(Seq<u8>, T) -> Seq<u8>> = Ghost(|acc: Seq<u8>, x: T| acc + x.ghost_serialize());
...
ensures
    ({let (rr, net_event) = rc;
        &&& netc.my_end_point() == old(netc).my_end_point()
        &&& netc.ok() == !rr.is_Fail()
        &&& !rr.is_Fail() ==> netc.ok() && netc.history() == old(netc).history() + seq!( net_event@ )
        ...
        &&& (rr.is_Packet() ==> {
            ...
            &&& !(rr.get_Packet_cpacket()@.msg is InvalidMessage) ==> {
                &&& rr.get_Packet_cpacket()@ == abstractify_net_packet_to_sht_packet(net_event@.get_Receive_r())
                &&& rr.get_Packet_cpacket().msg@ == sht_demarshal_data(net_event@.get_Receive_r().msg)@
            }
        ...
        NetcReceiveResult::Received{sender, message} => {
            ...
            proof {
                ...
                if !(gsinglemessage is InvalidMessage) {
                    ...
                    assert( cpacket@ =~= p );
                    assert( cpacket@ == abstractify_net_packet_to_sht_packet(net_event.get_Receive_r()) );
                    assert( gsinglemessage is Message ==> cpacket.msg@ == sht_demarshal_data(net_event.get_Receive_r().msg)@ );
                }
            }
...
ensures
    ...
    &&& ok ==> rc.1@.is_Some() && send_log_entry_reflects_packet(net_event.unwrap(), &cpacket)
...
{
    forall |i| 0 <= i < rawlog.len() && rawlog[i].is_Send() ==>
    ...
    })
{
            ...
            forall |i| 0 <= i < net_events.len() ==> net_events[i] is Send,
    {
        ...
        proof {
            ...
            assert forall |j| 0 <= j < net_events.len() && net_events[j].is_Send()
...
//         e
    ...
    pub open spec fn no_acks_in_unacked(list: Seq<CSingleMessage>) -> bool {
        forall |i: int| 0 <= i < list.len() ==> #[trigger] list[i].is_Message()
    }
        ...
        recommends Self::no_acks_in_unacked(list)
    {
        forall |i: int, j: int| #![auto] 0 <= i && j == i + 1 && j < list.len() ==>
            list[i].get_Message_seqno() as int + 1 == list[j].get_Message_seqno() as int
    }
    ...
    pub open spec fn un_acked_list_valid_for_dst(list: Seq<CSingleMessage>, dst: AbstractEndPoint) -> bool {
        ...
        &&& forall |i:int| 0 <= i < list.len() ==> (#[trigger] list[i].get_Message_dst())@ == dst
    }
        ...
        self.num_packets_acked == seqno_acked,
    {
        ...
        invariant
            ...
            self == old(self),
            i <= self.un_acked.len(),
            i < self.un_acked.len() ==> self.un_acked[i as int].get_Message_seqno() <= seqno_acked + 1,
            forall |j: int| #![auto] 0 <= j < i ==> self.un_acked[j].get_Message_seqno() <= seqno_acked,
...
{
    if un_acked.len() > 0 && un_acked[0] is Message && un_acked[0].get_Message_seqno() <= seqno_acked {
        ...
        &&& {
            ...
            if pkt.msg.get_Ack_ack_seqno() > old_ack_state.num_packets_acked {
                let new_ack_state = AckState{
                        ...
                        un_acked: truncate_un_ack_list(old_ack_state.un_acked, pkt.msg.get_Ack_ack_seqno()),
                        .. old_ack_state};
...
pub proof fn lemma_seq_fold_left_merge_right_assoc<A, B>(s: Seq<A>, init: B, f: FnSpec(A) -> B, g: FnSpec(B, B) -> B)
  requires
    s.len() > 0,
    forall |x, y, z|
      #[trigger g(x, y)]
...
pub proof fn lemma_seq_fold_left_append_right<A, B>(s: Seq<A>, prefix: Seq<B>, f: FnSpec(A) -> Seq<B>)
  requires s.len() > 0,
  ensures
...
pub proof fn lemma_seq_fold_left_append_len_int<A, B>(s: Seq<A>, prefix: Seq<B>, f: FnSpec(A) -> Seq<B>)
  ensures
...
pub proof fn lemma_seq_fold_left_sum_len_int_positive<A, B>(s: Seq<A>, low: nat, f: FnSpec(A) -> Seq<B>)
  ensures
...
pub proof fn lemma_seq_fold_left_append_len_int_le<A, B>(s: Seq<A>, i: int, low: int, f: FnSpec(A) -> Seq<B>)
  requires
    0 <= i <= s.len() as int,
    0 <= low,
  ensures
...
pub proof fn lemma_if_everything_in_seq_satisfies_filter_then_filter_is_identity<A>(s: Seq<A>, pred: FnSpec(A) -> bool)
...
pub proof fn lemma_if_nothing_in_seq_satisfies_filter_then_filter_result_is_empty<A>(s: Seq<A>, pred: FnSpec(A) -> bool)
    requires forall |i: int| 0 <= i && i < s.len() ==> !pred(s[i])
...
pub proof fn lemma_fold_left_on_equiv_seqs<A, B>(s1: Seq<A>, s2: Seq<A>, eq: FnSpec(A, A) -> bool, init: B, f: FnSpec(B, A) -> B)
    requires
      s1.len() == s2.len(),
...
pub proof fn set_map_union_auto<A, B>()
    ensures forall |s1: Set<A>, s2: Set<A>, f: FnSpec(A) -> B|
...
ensures forall |s1: Seq<A>, s2: Seq<A>, f: FnSpec(A) -> B|
...
ensures
    forall |s: Set<A>, f: FnSpec(A) -> B| s.finite() ==> #[trigger] (s.map(f).finite()),
{
    assert forall |s: Set<A>, f: FnSpec(A) -> B| s.finite() implies #[trigger] s.map(f).finite() by {
        map_finite(s, f);
    }
}
