        ...
        (i % len) == i,
{
    ...
    pub mod linearization {
        ...
        use crate::{
            ...
            InputOperation, OutputOperation, SimpleLogBehavior,
            SimpleLogRefinesAsynchronousSingleton,
        };
...
pub ghost enum ReadonlyState<DT: Dispatch> {
    ...
    Done { op: DT::ReadOperation, ret: DT::Response, node_id: NodeId, version_upper_bound: LogIdx },
}
...
pub open spec fn LogRangeMatchesQueue2<DT: Dispatch>(
    queue: Seq<ReqId>,
    log: Map<LogIdx, LogEntry<DT>>,
    queueIndex: nat,
    logIndexLower: LogIdx,
...
// end verus!
    ...
    pub mod unbounded_log_refines_simplelog {
        ...
        use super::unbounded_log::{
            compute_nrstate_at_version as i_nrstate_at_version, ReadonlyState, UnboundedLog,
            UpdateState,
        };
        use super::utils::*;
...
pub tracked struct RwLockWriteGuard<
    #[cfg_attr(verus_keep_ghost, verifier::maybe_negative)]
    T,
> {
...
struct_with_invariants!{
    ...
    pub mod replica {
        ...
        use crate::exec::utils::{
            rids_match, rids_match_add_none, rids_match_add_rid, rids_match_pop,
        };
        use crate::exec::CachePadded;
...
impl DataStructureSpec {
    ...
    pub open spec fn spec_update(self, op: UpdateOp) -> (Self, ReturnType) {
        match op {
            ...
            UpdateOp::Inc => (
                DataStructureSpec {
                    val: if self.val < 0xffff_ffff_ffff_ffff {
                        (self.val + 1) as u64
                    } else {
                        0
                    },
...
impl DataStructureType {
    pub fn init() -> (result: Self)
        ensures
            result.interp() == DataStructureSpec::init(),
    {
    ...
    pub fn update(&mut self, op: UpdateOp) -> (result: ReturnType)
        ensures
            old(self).interp().spec_update(op) == (self.interp(), result),
    {
        // print_update_op(&op, self.val);
        match op {
            UpdateOp::Reset => self.val = 0,
            UpdateOp::Inc => self.val = if self.val < 0xffff_ffff_ffff_ffff {
                self.val + 1
            } else {
                0
            },
    ...
    pub fn read(&self, op: ReadonlyOp) -> (result: ReturnType)
        ensures
            self.interp().spec_read(op) == result,
    {
...
impl Dispatch for DataStructureType {
    ...
    fn dispatch(&self, op: Self::ReadOperation) -> (result: Self::Response) {
        match op {
            ReadonlyOp::Get => { ReturnType::Value(self.val) },
        }
    }
    ...
    open spec fn dispatch_spec(ds: Self::View, op: Self::ReadOperation) -> Self::Response {
        match op {
            ReadonlyOp::Get => { ReturnType::Value(ds.val) },
        }
    }
    ...
    open spec fn dispatch_mut_spec(ds: Self::View, op: Self::WriteOperation) -> (
        ...
        match op {
            ...
            UpdateOp::Inc => (
                DataStructureSpec {
                    val: if ds.val < 0xffff_ffff_ffff_ffff {
                        (ds.val + 1) as u64
                    } else {
                        0
                    },
    ...
    pub fn example_main() {
        ...
        for idx in 0..NUM_REPLICAS {
            ...
            match nr_counter.execute(ReadonlyOp::Get, tkn, Tracked::assume_new()) {
                Result::Ok((ret, t, _)) => match ret {
                    ReturnType::Value(v) => {
                        println!(
                            "Replica {idx} - Final Result: {v} expected {}",
                            NUM_THREADS * (NUM_OPS_PER_THREAD) / 2
                        );
                    }
                    ReturnType::Ok => {
                        println!(
                            "Replica {idx} - Final Result: OK? expected {}",
                            NUM_THREADS * (NUM_OPS_PER_THREAD) / 2
                        );
                    }
                },
...
#[verus::trusted]
spec fn implements_NodeReplicated<DT: Dispatch + Sync, N: NR<DT>>() -> bool {
    true
}
