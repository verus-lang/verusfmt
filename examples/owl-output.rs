...
pub fn owl_output<A>(
    ...
    requires
        old(t)@.is_output(x@, endpoint_of_addr(dest_addr.view())),
    ensures
        t@ == old(t)@.give_output(),
{
    let msg = msg {
...
pub fn owl_input<A>(
    ...
    requires
        old(t)@.is_input(),
    ensures
        t@ == old(t)@.take_input(ie.0@, endpoint_of_addr(ie.1.view())),
{
...
#[verifier(external_body)]
pub closed spec fn parse_owlSpec_t(x: Seq<u8>) -> Option<owlSpec_t> {
    todo!()
}

#[verifier(external_body)]
pub closed spec fn serialize_owlSpec_t(x: owlSpec_t) -> Seq<u8> {
    todo!()
}
...
#[verifier(external_body)]
pub closed spec fn parse_owlSpec_Result(x: Seq<u8>) -> Option<owlSpec_Result> {
    todo!()
}

#[verifier(external_body)]
pub closed spec fn serialize_owlSpec_Result(x: owlSpec_Result) -> Seq<u8> {
    todo!()
}
...
pub exec fn parse_owl_t(arg: &[u8]) -> (res: Option<owl_t>)
    ensures
        res.is_Some() ==> parse_owlSpec_t(arg.view()).is_Some(),
        res.is_None() ==> parse_owlSpec_t(arg.view()).is_None(),
        res.is_Some() ==> res.get_Some_0().owl__x.view() == parse_owlSpec_t(
            arg.view(),
        ).get_Some_0().owlSpec__x,
        res.is_Some() ==> res.get_Some_0().owl__y.view() == parse_owlSpec_t(
            arg.view(),
        ).get_Some_0().owlSpec__y,
{
...
pub exec fn serialize_owl_t(arg: &owl_t) -> (res: Vec<u8>)
    ensures
        res.view() == serialize_owlSpec_t(
            owlSpec_t { owlSpec__x: arg.owl__x.view(), owlSpec__y: arg.owl__y.view() },
        ),
{
...
pub struct owl_Result {
    pub data: Rc<Vec<u8>>,
    pub parsing_outcome: owl_Result_ParsingOutcome,
}
...
#[verifier(external_body)]
pub fn len_valid_owl_Result(arg: &[u8]) -> Option<usize> {
    if arg.len() < 1 {
        return None;
    } else if *slice_index_get(arg, 0) == 1u8 && arg.len() >= 1 {
        return Some(1);
    } else if *slice_index_get(arg, 0) == 2u8 && arg.len() >= 13 {
        return Some(13);
    } else {
        return None;
    }
...
#[verifier(external_body)]
pub fn parse_into_owl_Result(arg: &mut owl_Result) {
...
#[verifier(external_body)]
pub fn construct_owl_Result_owl_NoResult() -> (res: owl_Result)
    ensures
        res.data.view() === NoResult(),
{
...
#[verifier(external_body)]
pub fn construct_owl_Result_owl_SomeResult(arg: &[u8]) -> (res: owl_Result)
    ensures
        res.data.view() === SomeResult(arg@),
{
    let mut v = vec_u8_from_elem(2u8, 1);
    if arg.len() < 12 {
        return owl_Result {
            data: rc_new(vec_u8_from_elem(0, 1)),
            parsing_outcome: owl_Result_ParsingOutcome::Failure,
        };
    }
...
#[verifier(external_body)]
pub const fn alice_addr() -> (a: StrSlice<'static>)
    ensures
...
impl cfg_alice {
    #[verifier(external_body)]
    pub fn init_cfg_alice(config_path: &StrSlice) -> Self {
        ...
        return cfg_alice {
            listener,
            owl_k_data: rc_new(owl_k_data),
            owl_shared_key: rc_new(config.owl_shared_key),
            salt: rc_new(config.salt),
        }
    }

    pub fn owl_alice_main(
        &self,
    ...
    >)
        ...
        ensures
            res.is_Ok() ==> (res.get_Ok_0().1)@@.results_in(
                (res.get_Ok_0().0.data.view(), *mut_state),
            ),
    {
        ...
        let res_inner = {
            ...
            let temp_owl__x8 = {
                let coins = owl_sample::<(Seq<u8>, state_alice)>(Tracked(&mut itree), nonce_size());
                owl_enc(
                    &(*rc_clone(&owl__x5)).as_slice(),
                    &(*rc_clone(&owl__x7)).as_slice(),
                    &coins.as_slice(),
                )
            };
            ...
            let temp_owl__x13 = {
                owl_output::<(Seq<u8>, state_alice)>(
                    Tracked(&mut itree),
                    &(*rc_clone(&owl__x12)).as_slice(),
                    &bob_addr(),
                    &alice_addr(),
                )
            };
            let owl__x13 = temp_owl__x13;
            let (temp_owl_i15, owl__14) = owl_input::<(Seq<u8>, state_alice)>(
                Tracked(&mut itree),
                &self.listener,
            );
            ...
            let temp_owl__x39 = {
                owl_dec(&(*rc_clone(&owl__x35)).as_slice(), &(*rc_clone(&owl__x37)).as_slice())
            };
            ...
            let temp_owl__x40 = {
                ...
                match owl_caseval42 {
                    Some(temp_owl_j19) => {
                        let owl_j19 = rc_clone(&temp_owl_j19);
                        ...
                        if let Some(parseval) = parse_owl_t(&(*rc_clone(&owl__x27)).as_slice()) {
                            ...
                            let temp_owl__x26 = {
                                construct_owl_Result_owl_SomeResult(
                                    &(*rc_clone(&owl__x25)).as_slice(),
                                )
                            };
    ...
    #[verifier(external_body)]
    pub exec fn owl_alice_main_wrapper(&self, s: &mut state_alice) -> (_: owl_Result) {
        ...
        let (res, _): (owl_Result, Tracked<ITreeToken<(Seq<u8>, state_alice), Endpoint>>) =
            self.owl_alice_main(Tracked(call_token), s  /* todo args? */ ).unwrap();
        res
    }
}
...
impl cfg_bob {
    #[verifier(external_body)]
    pub fn init_cfg_bob(config_path: &StrSlice) -> Self {
            ...
            res.is_Ok() ==> (res.get_Ok_0().1)@@.results_in(((), *mut_state)),
    {
        let tracked mut itree = itree;
        let res_inner = {
            let (temp_owl_i45, owl_ev44) = owl_input::<((), state_bob)>(
                Tracked(&mut itree),
                &self.listener,
            );
            ...
            let temp_owl__x78 = {
                owl_dec(&(*rc_clone(&owl__x75)).as_slice(), &(*rc_clone(&owl__x77)).as_slice())
            };
            ...
            match owl_caseval79 {
                Some(temp_owl_k48) => {
                    let owl_k48 = rc_clone(&temp_owl_k48);
                    ...
                    let temp_owl__x62 = {
                        owl_t {
                            owl__x: clone_vec_u8(&*rc_clone(&owl__x58)),
                            owl__y: clone_vec_u8(&*rc_clone(&owl__x60)),
                        }
                    };
                    ...
                    let temp_owl__x71 = {
                        let coins = owl_sample::<((), state_bob)>(
                            Tracked(&mut itree),
                            nonce_size(),
                        );
                        owl_enc(
                            &(*rc_clone(&owl__x68)).as_slice(),
                            &(serialize_owl_t(&owl__x70)).as_slice(),
                            &coins.as_slice(),
                        )
                    };
                    ...
                    (
                        owl_output::<((), state_bob)>(
                            Tracked(&mut itree),
                            &(*rc_clone(&owl__x72)).as_slice(),
                            &owl_ev44.as_str(),
                            &bob_addr(),
                        ),
                        Tracked(itree),
    ...
    #[verifier(external_body)]
    pub exec fn owl_bob_main_wrapper(&self, s: &mut state_bob) -> (_: ()) {
        let tracked dummy_tok: ITreeToken<(), Endpoint> = ITreeToken::<
            (),
            Endpoint,
        >::dummy_itree_token();
        let tracked (Tracked(call_token), _) = split_bind(dummy_tok, bob_main_spec(*self, *s));
        let (res, _): ((), Tracked<ITreeToken<((), state_bob), Endpoint>>) = self.owl_bob_main(
            Tracked(call_token),
            s,  /* todo args? */
        ).unwrap();
        res
...
fn entrypoint() {
    ...
    if args.len() >= 4 && args[1] == "run" && args[2] == "alice" {
        let loc = cfg_alice::init_cfg_alice(&String::from_rust_string(args[3].clone()).as_str());
    ...
    } else if args.len() >= 4 && args[1] == "run" && args[2] == "bob" {
        let loc = cfg_bob::init_cfg_bob(&String::from_rust_string(args[3].clone()).as_str());
        ...
        println!("Elapsed: {:?}", elapsed);
    } else if args.len() >= 3 && args[1] == "config" {
        let owl_shared_key = owl_aead::gen_rand_key(cipher());
        let salt = owl_util::gen_rand_bytes(64);
        let cfg_alice_config: cfg_alice_config = cfg_alice_config {
            owl_shared_key: owl_shared_key.clone(),
            salt: salt.clone(),
        };
        let cfg_alice_config_serialized = serialize_cfg_alice_config(&cfg_alice_config);
        let mut cfg_alice_f = fs::File::create(
            format!("{}/{}.owl_config", &args[2], "cfg_alice"),
        ).expect("Can't create config file");
        cfg_alice_f.write_all(cfg_alice_config_serialized.as_bytes()).expect(
            "Can't write config file",
        );
        let cfg_bob_config: cfg_bob_config = cfg_bob_config {
            owl_shared_key: owl_shared_key.clone(),
            salt: salt.clone(),
        };
        let cfg_bob_config_serialized = serialize_cfg_bob_config(&cfg_bob_config);
        let mut cfg_bob_f = fs::File::create(
            format!("{}/{}.owl_config", &args[2], "cfg_bob"),
        ).expect("Can't create config file");
        cfg_bob_f.write_all(cfg_bob_config_serialized.as_bytes()).expect("Can't write config file");
    } else {
