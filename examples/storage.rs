...
pub open spec fn pm_to_views(pm: Seq<u8>) -> (u64, HeaderView, Seq<u8>) {
    let incorruptible_bool = spec_u64_from_le_bytes(
    ...
    let crc1 = spec_u64_from_le_bytes(
        pm.subrange(header1_pos + header_crc_offset, header1_pos + header_crc_offset + 8),
    );
    let crc2 = spec_u64_from_le_bytes(
        pm.subrange(header2_pos + header_crc_offset, header2_pos + header_crc_offset + 8),
    );
    let header1_metadata = spec_bytes_to_metadata(
        pm.subrange(header1_pos + header_head_offset, header1_pos + header_size),
    );
    let header2_metadata = spec_bytes_to_metadata(
        pm.subrange(header2_pos + header_head_offset, header2_pos + header_size),
    );
    let header_view = HeaderView {
        header1: PersistentHeader { crc: crc1, metadata: header1_metadata },
        header2: PersistentHeader { crc: crc2, metadata: header2_metadata },
    };
    let data_view = pm.subrange(contents_offset as int, pm.len() as int);
    (incorruptible_bool, header_view, data_view)
}
...
pub open spec fn permissions_depend_only_on_recovery_view<Perm: CheckPermission<Seq<u8>>>(
    perm: &Perm,
) -> bool {
    forall|s1, s2|
        recovery_view()(s1) == recovery_view()(s2) ==> perm.check_permission(s1)
            == perm.check_permission(s2)
}

pub proof fn lemma_same_permissions<Perm: CheckPermission<Seq<u8>>>(
    pm1: Seq<u8>,
    pm2: Seq<u8>,
    perm: &Perm,
)
...
pub proof fn lemma_header_match(pm: Seq<u8>, header_pos: int, header: PersistentHeader)
    ...
    ensures
        ({
            ...
            &&& header_pos == header1_pos ==> headers.header1 == header
            &&& header_pos == header2_pos ==> headers.header2 == header
        }),
{
    assert(pm.subrange(header_pos as int, header_pos + header_size) =~= pm.subrange(
        header_pos + header_crc_offset,
        header_pos + header_crc_offset + 8,
...
pub proof fn lemma_bytes_combine_into_header(
    crc_bytes: Seq<u8>,
    metadata_bytes: Seq<u8>,
    ...
    ensures
        ({
            let combined_header = PersistentHeader {
                crc: spec_u64_from_le_bytes(crc_bytes),
                metadata: spec_bytes_to_metadata(metadata_bytes),
            };
        ...
        }),
{
    ...
    assert(metadata == spec_bytes_to_metadata(
        (crc_bytes + metadata_bytes).subrange(header_head_offset as int, header_size as int),
    ));
...
pub proof fn lemma_header_split_into_bytes(
    crc_bytes: Seq<u8>,
    metadata_bytes: Seq<u8>,
    header_bytes: Seq<u8>,
)
        ...
        }),
{
    ...
    assert(forall|i: int|
        #![auto]
        0 <= i < bytes2.len() ==> (bytes1 + bytes2)[bytes1.len() + i] == bytes2[i]);
}
...
{
    let crc_bytes = slice_subrange(
        bytes,
        header_crc_offset as usize,
        (header_crc_offset + 8) as usize,
    );
...
exec fn header_to_bytes(header: &PersistentHeader) -> (out: Vec<u8>)
    ensures
        ...
        spec_u64_from_le_bytes(out@.subrange(header_crc_offset as int, header_crc_offset + 8))
            == header.crc,
        spec_bytes_to_metadata(out@.subrange(header_head_offset as int, header_size as int))
            == header.metadata,
        out@.len() == header_size,
{
    proof {
    ...
    proof {
        lemma_auto_spec_u64_to_from_le_bytes();
        assert(old_crc_bytes =~= crc_bytes@.subrange(
            header_crc_offset as int,
            header_crc_offset + 8,
        ));
        assert(old_metadata_bytes =~= crc_bytes@.subrange(
            header_head_offset as int,
            header_size as int,
        ));
    }
    crc_bytes
...
{
    let head_bytes = slice_subrange(
        bytes,
        (header_head_offset - 8) as usize,
        (header_head_offset - 8 + 8) as usize,
    );
    let tail_bytes = slice_subrange(
        bytes,
        (header_tail_offset - 8) as usize,
        (header_tail_offset - 8 + 8) as usize,
    );
    let log_size_bytes = slice_subrange(
        bytes,
        (header_log_size_offset - 8) as usize,
        (header_log_size_offset - 8 + 8) as usize,
    );
        ...
        out@.len() == header_size - 8,
{
    ...
    proof {
        ...
        assert(old_head_bytes =~= bytes@.subrange(
            header_head_offset - 8,
            header_head_offset - 8 + 8,
        ));
        assert(old_tail_bytes =~= bytes@.subrange(
            header_tail_offset - 8,
            header_tail_offset - 8 + 8,
        ));
        assert(old_log_size_bytes =~= bytes@.subrange(
            header_log_size_offset - 8,
            header_log_size_offset - 8 + 8,
        ));
    }
    bytes
}
...
pub open spec(checked) fn spec_bytes_to_metadata(header_seq: Seq<u8>) -> PersistentHeaderMetadata
    recommends
        header_seq.len() == 3 * 8,
{
    let head = spec_u64_from_le_bytes(
        header_seq.subrange(header_head_offset - 8, header_head_offset - 8 + 8),
    );
    let tail = spec_u64_from_le_bytes(
        header_seq.subrange(header_tail_offset - 8, header_tail_offset - 8 + 8),
    );
    let log_size = spec_u64_from_le_bytes(
        header_seq.subrange(header_log_size_offset - 8, header_log_size_offset - 8 + 8),
    );
    PersistentHeaderMetadata { head, tail, log_size }
}
...
pub proof fn lemma_metadata_bytes_eq(
    bytes1: Seq<u8>,
    bytes2: Seq<u8>,
    metadata: PersistentHeaderMetadata,
)
        ...
        bytes1 =~= bytes2,
{
    ...
    assert(metadata1.head == spec_u64_from_le_bytes(
        bytes1.subrange(header_head_offset - 8, header_head_offset - 8 + 8),
    ));
    assert(metadata2.head == spec_u64_from_le_bytes(
        bytes2.subrange(header_head_offset - 8, header_head_offset - 8 + 8),
    ));
    assert(bytes1.subrange(header_head_offset - 8, header_head_offset - 8 + 8) =~= bytes2.subrange(
        header_head_offset - 8,
        header_head_offset - 8 + 8,
    ));
    assert(spec_u64_to_le_bytes(metadata1.tail) == spec_u64_to_le_bytes(metadata2.tail));
    assert(metadata1.tail == spec_u64_from_le_bytes(
        bytes1.subrange(header_tail_offset - 8, header_tail_offset - 8 + 8),
    ));
    assert(metadata2.tail == spec_u64_from_le_bytes(
        bytes2.subrange(header_tail_offset - 8, header_tail_offset - 8 + 8),
    ));
    assert(bytes1.subrange(header_tail_offset - 8, header_tail_offset - 8 + 8) =~= bytes2.subrange(
        header_tail_offset - 8,
        header_tail_offset - 8 + 8,
    ));
    assert(spec_u64_to_le_bytes(metadata1.log_size) == spec_u64_to_le_bytes(metadata2.log_size));
    assert(metadata1.log_size == spec_u64_from_le_bytes(
        bytes1.subrange(header_log_size_offset - 8, header_log_size_offset - 8 + 8),
    ));
    assert(metadata2.log_size == spec_u64_from_le_bytes(
        bytes2.subrange(header_log_size_offset - 8, header_log_size_offset - 8 + 8),
    ));
    assert(bytes1.subrange(header_log_size_offset - 8, header_log_size_offset - 8 + 8)
        =~= bytes2.subrange(header_log_size_offset - 8, header_log_size_offset - 8 + 8));
    assert(bytes1 =~= bytes1.subrange(header_head_offset - 8, header_head_offset - 8 + 8)
        + bytes1.subrange(header_tail_offset - 8, header_tail_offset - 8 + 8) + bytes1.subrange(
        header_log_size_offset - 8,
        header_log_size_offset - 8 + 8,
    ));
}
...
{
    let crc_val = spec_u64_from_le_bytes(
        header_seq.subrange(header_crc_offset as int, header_crc_offset + 8),
    );
    let metadata = spec_bytes_to_metadata(
        header_seq.subrange(header_head_offset as int, header_size as int),
    );
    PersistentHeader { crc: crc_val, metadata }
}

/// Proves that a write to data that does not touch any metadata is crash safe.
pub proof fn lemma_data_write_is_safe<Perm>(
    pm: Seq<u8>,
    bytes: Seq<u8>,
    write_addr: int,
    perm: &Perm,
    ...
    requires
        ...
        ({
            ...
            let physical_head = spec_addr_logical_to_physical(
                live_header.metadata.head as int,
                live_header.metadata.log_size as int,
            );
            let physical_tail = spec_addr_logical_to_physical(
                live_header.metadata.tail as int,
                live_header.metadata.log_size as int,
            );
...
pub open spec fn update_data_view_postcond(
    ...
    &&& new_data.subrange(
        write_addr - contents_offset,
        write_addr - contents_offset + new_bytes.len(),
    ) =~= new_bytes
    &&& new_data.subrange(0, write_addr - contents_offset) =~= old_data.subrange(
        0,
        write_addr - contents_offset,
    )
    ...
    &&& physical_head < physical_tail ==> new_data.subrange(
        physical_head - contents_offset,
        physical_tail - contents_offset,
    ) =~= old_data.subrange(physical_head - contents_offset, physical_tail - contents_offset)
    &&& physical_tail < physical_head ==> {
        &&& old_data.subrange(physical_head - contents_offset, live_header.metadata.log_size as int)
            =~= new_data.subrange(
            physical_head - contents_offset,
            live_header.metadata.log_size as int,
        )
        &&& old_data.subrange(0, physical_tail - contents_offset) =~= new_data.subrange(
            0,
            physical_tail - contents_offset,
        )
    }
        ...
        update_data_view_postcond(pm, new_bytes, write_addr),
{
    ...
    assert(new_pm.subrange(write_addr + new_bytes.len(), new_pm.len() as int) =~= pm.subrange(
        write_addr + new_bytes.len(),
        pm.len() as int,
    ));
    lemma_subrange_equality_implies_subsubrange_equality(pm, new_pm, 0, write_addr);
    lemma_subrange_equality_implies_subsubrange_equality(
        pm,
        new_pm,
        write_addr + new_bytes.len(),
        new_pm.len() as int,
    ...
    if physical_head < physical_tail {
        assert(new_pm.subrange(physical_head as int, physical_tail as int) =~= pm.subrange(
            physical_head as int,
            physical_tail as int,
        ));
    }
}

/// Proves that a crashing data write updates data bytes but no log metadata.
pub proof fn lemma_append_data_update_view_crash(
    pm: Seq<u8>,
    new_bytes: Seq<u8>,
    write_addr: int,
    chunks_flushed: Set<int>,
)
    requires
        UntrustedLogImpl::recover(pm).is_Some(),
        pm.len() > contents_offset,
        ...
        ({
            ...
            &&& physical_head <= physical_tail ==> write_addr + new_bytes.len()
                <= live_header.metadata.log_size + contents_offset
...
pub proof fn lemma_inactive_header_update_view(
    pm: Seq<u8>,
    ...
    ensures
        ({
            ...
            &&& old_ib == new_ib
            &&& old_data =~= old_data
            &&& header_pos == header1_pos ==> old_headers.header2 == new_headers.header2
            &&& header_pos == header2_pos ==> old_headers.header1 == new_headers.header1
            &&& UntrustedLogImpl::recover(new_pm).is_Some()
        }),
{
    let new_pm = update_contents_to_reflect_write(pm, header_pos, new_header_bytes);
    let (new_ib, new_headers, new_data) = pm_to_views(new_pm);
    assert(pm.subrange(incorruptible_bool_pos as int, incorruptible_bool_pos + 8)
        =~= new_pm.subrange(incorruptible_bool_pos as int, incorruptible_bool_pos + 8));
    if header_pos == header1_pos {
        ...
        assert(pm.subrange(header2_pos + header_crc_offset, header2_pos + header_crc_offset + 8)
            =~= new_pm.subrange(
            header2_pos + header_crc_offset,
            header2_pos + header_crc_offset + 8,
        ));
        ...
        assert(pm.subrange(header1_pos + header_crc_offset, header1_pos + header_crc_offset + 8)
            =~= new_pm.subrange(
            header1_pos + header_crc_offset,
            header1_pos + header_crc_offset + 8,
        ));
...
pub proof fn lemma_inactive_header_update_view_crash(
    pm: Seq<u8>,
    new_header_bytes: Seq<u8>,
    header_pos: int,
    chunks_flushed: Set<int>,
...
pub proof fn lemma_append_ib_update<Perm: CheckPermission<Seq<u8>>>(
    ...
    requires
        ...
        new_ib == cdb0_val ==> pm.subrange(header1_pos as int, header1_pos + header_size)
            == new_header_bytes,
        new_ib == cdb1_val ==> pm.subrange(header2_pos as int, header2_pos + header_size)
            == new_header_bytes,
        new_header_bytes.subrange(header_crc_offset as int, header_crc_offset + 8)
            == spec_crc_bytes(
            new_header_bytes.subrange(header_head_offset as int, header_size as int),
        ),
        ({
        ...
        ({
            ...
            &&& physical_tail + append_size >= contents_end ==> {
                ...
                &&& pm.subrange(contents_offset as int, contents_offset + len2)
                    =~= bytes_to_append.subrange(len1 as int, append_size as int)
                &&& bytes_to_append =~= pm.subrange(physical_tail, contents_end) + pm.subrange(
                    contents_offset as int,
                    contents_offset + len2,
                )
            }
        ...
        ({
            ...
            forall|pm_state| #[trigger]
                perm.check_permission(pm_state) <==> {
                    let log_state = UntrustedLogImpl::recover(pm_state);
                    log_state == old_log_state || log_state == Some(
                        old_log_state.unwrap().append(bytes_to_append),
                    )
                }
        }),
    ensures
        ({
            ...
            let new_pm = update_contents_to_reflect_write(
                pm,
                incorruptible_bool_pos as int,
                ib_bytes,
            );
            let old_log_state = UntrustedLogImpl::recover(pm);
            ...
            },
{
    ...
    match (new_log_state, old_log_state) {
        (Some(new_log_state), Some(old_log_state)) => {
            ...
            let new_physical_tail = spec_addr_logical_to_physical(
                live_header.metadata.tail as int,
                live_header.metadata.log_size as int,
            );
            let old_physical_tail = spec_addr_logical_to_physical(
                old_header.metadata.tail as int,
                old_header.metadata.log_size as int,
            );
            ...
            if physical_head <= old_physical_tail {
                if old_physical_tail + append_size >= contents_end {
                    assert(new_log_state.log =~= new_data.subrange(
                        physical_head - contents_offset,
                        old_physical_tail - contents_offset,
                    ) + new_data.subrange(
                        old_physical_tail - contents_offset,
                        contents_end - contents_offset,
                    ) + new_data.subrange(0, new_physical_tail - contents_offset));
                    assert(new_log_state.log =~= old_data.subrange(
                        physical_head - contents_offset,
                        old_physical_tail - contents_offset,
                    ) + new_data.subrange(
                        old_physical_tail - contents_offset,
                    ...
                    let len2 = bytes_to_append.len() - len1;
                    assert(bytes_to_append =~= new_data.subrange(
                        old_physical_tail - contents_offset,
                        contents_end - contents_offset,
                    ) + new_data.subrange(0, new_physical_tail - contents_offset));
                    assert(new_log_state.log =~= old_data.subrange(
                    ...
                    assert(old_data.subrange(0, old_physical_tail - contents_offset)
                        =~= new_data.subrange(0, old_physical_tail - contents_offset));
                    assert(new_data.subrange(
                        old_physical_tail - contents_offset,
                        old_physical_tail - contents_offset + append_size,
                    ) =~= bytes_to_append);
                }
...
pub open spec fn live_data_view_eq(old_pm: Seq<u8>, new_pm: Seq<u8>) -> bool {
    ...
    let new_live_header = spec_get_live_header(new_pm);
    let physical_head = spec_addr_logical_to_physical(
        old_live_header.metadata.head as int,
        old_live_header.metadata.log_size as int,
    );
    let physical_tail = spec_addr_logical_to_physical(
        old_live_header.metadata.tail as int,
        old_live_header.metadata.log_size as int,
    );
    ...
    &&& physical_head < physical_tail ==> old_data.subrange(physical_data_head, physical_data_tail)
        =~= new_data.subrange(physical_data_head, physical_data_tail)
    &&& physical_tail < physical_head ==> {
        &&& old_data.subrange(physical_data_head as int, log_size as int) =~= new_data.subrange(
            physical_data_head as int,
            log_size as int,
        )
        &&& old_data.subrange(0, physical_data_tail as int) =~= new_data.subrange(
            0,
            physical_data_tail as int,
        )
    }
...
pub proof fn lemma_same_log_state(old_pm: Seq<u8>, new_pm: Seq<u8>)
    ...
    ensures
        UntrustedLogImpl::recover(old_pm) =~= UntrustedLogImpl::recover(new_pm),
{
    ...
    match (old_state, new_state) {
        (Some(old_state), Some(new_state)) => {
            ...
            if physical_head < physical_tail {
                assert(old_pm.subrange(physical_head, physical_tail) =~= old_data.subrange(
                    physical_head - contents_offset,
                    physical_tail - contents_offset,
                ));
                assert(old_pm.subrange(physical_head, physical_tail) =~= new_pm.subrange(
                    physical_head,
                    physical_tail,
                ));
            } else if physical_tail < physical_head {
                assert(old_pm.subrange(physical_head, contents_end) =~= old_data.subrange(
                    physical_head - contents_offset,
                    contents_end - contents_offset,
                ));
                assert(old_pm.subrange(contents_offset as int, physical_tail) =~= old_data.subrange(
                    contents_offset - contents_offset,
                    physical_tail - contents_offset,
                ));
                assert(old_pm.subrange(physical_head, contents_end) + old_pm.subrange(
                    contents_offset as int,
                    physical_tail,
                ) =~= new_pm.subrange(physical_head, contents_end) + new_pm.subrange(
                    contents_offset as int,
                    physical_tail,
                ));
            } else {
...
pub proof fn lemma_subrange_equality_implies_index_equality<T>(
    s1: Seq<T>,
    s2: Seq<T>,
    i: int,
    j: int,
...
pub proof fn lemma_subrange_equality_implies_subsubrange_equality<T>(
    s1: Seq<T>,
    s2: Seq<T>,
    i: int,
    j: int,
...
pub proof fn lemma_headers_unchanged(old_pm: Seq<u8>, new_pm: Seq<u8>)
    requires
        ...
        old_pm.subrange(header1_pos as int, header1_pos + header_size) =~= new_pm.subrange(
            header1_pos as int,
            header1_pos + header_size,
        ),
        old_pm.subrange(header2_pos as int, header2_pos + header_size) =~= new_pm.subrange(
            header2_pos as int,
            header2_pos + header_size,
        ),
    ensures
        ({
...
pub proof fn lemma_incorruptible_bool_unchanged(old_pm: Seq<u8>, new_pm: Seq<u8>)
    requires
        old_pm.len() == new_pm.len(),
        old_pm.len() >= contents_offset,
        old_pm.subrange(incorruptible_bool_pos as int, incorruptible_bool_pos + 8)
            =~= new_pm.subrange(incorruptible_bool_pos as int, incorruptible_bool_pos + 8),
    ensures
        ({
...
pub proof fn lemma_header_crc_correct(
    header_bytes: Seq<u8>,
    crc_bytes: Seq<u8>,
    metadata_bytes: Seq<u8>,
)
    ...
    ensures
        ...
        header_bytes.subrange(header_crc_offset as int, header_crc_offset + 8) =~= spec_crc_bytes(
            header_bytes.subrange(header_head_offset as int, header_size as int),
        ),
{
...
pub proof fn lemma_subrange_eq<T>(bytes1: Seq<T>, bytes2: Seq<T>)
    ...
    ensures
        forall|i: int, j: int|
            0 <= i < j < bytes1.len() ==> bytes1.subrange(i, j) =~= bytes2.subrange(i, j),
{
}
            ...
            forall|chunks_flushed: Set<int>|
                {
                    ...
                    let new_contents = update_contents_to_reflect_write(
                        pm,
                        write_addr,
                        bytes_to_write,
                    );
...
impl UntrustedLogImpl {
    ...
    pub open spec fn log_state_is_valid(pm: Seq<u8>) -> bool {
        ...
        &&& tail - head < log_size
        &&& ib == cdb0_val ==> {
            &&& live_header.crc == spec_u64_from_le_bytes(
                spec_crc_bytes(
                    pm.subrange(header1_pos + header_head_offset, header1_pos + header_size),
                ),
            )
    ...
    pub open spec fn inv_pm_contents(self, contents: Seq<u8>) -> bool {
        ...
        &&& ib == cdb0_val || ib == cdb1_val
        &&& spec_crc_bytes(
            contents.subrange(header_pos + header_head_offset, header_pos + header_size),
    ...
    ) where Perm: CheckPermission<Seq<u8>>, PM: PersistentMemory
        requires
            permissions_depend_only_on_recovery_view(perm),
            ...
            ({
                ...
                &&& old_pm_ib == cdb0_val ==> {
                    ...
                    &&& wrpm@.subrange(
                        ...
                        wrpm@.subrange(header2_pos + header_head_offset, header2_pos + header_size),
                    )
                    &&& wrpm@.subrange(header2_pos as int, header2_pos + header_size)
                        =~= new_header_bytes@
                }
                &&& old_pm_ib == cdb1_val ==> {
                    ...
                    &&& wrpm@.subrange(
                        ...
                        wrpm@.subrange(header1_pos + header_head_offset, header1_pos + header_size),
                    )
                    &&& wrpm@.subrange(header1_pos as int, header1_pos + header_size)
                        =~= new_header_bytes@
                }
                &&& old_data =~= new_data
            }),
    {
        ...
        proof {
            ...
            assert forall|chunks_flushed|
                ...
                lemma_inactive_header_update_view_crash(
                    wrpm@,
                    new_header_bytes@,
                    header_pos as int,
                    chunks_flushed,
        ...
        proof {
            ...
            assert(wrpm@.subrange(header_pos as int, header_pos + header_size)
                =~= new_header_bytes@);
            ...
            assert(old(wrpm)@.subrange(
                live_header_pos + header_crc_offset,
                live_header_pos + header_crc_offset + 8,
            ) =~= spec_crc_bytes(
                old(wrpm)@.subrange(
                    live_header_pos + header_head_offset,
                    live_header_pos + header_size,
                ),
            ));
            assert(old(wrpm)@.subrange(
                live_header_pos + header_crc_offset,
    ...
    >) where PM: PersistentMemory
        ...
        ensures
            ...
            pm@.len() == device_size,
            match result {
                Ok(capacity) => Self::recover(pm@) == Some(
                    AbstractInfiniteLogState::initialize(capacity as int),
                ),
                Err(InfiniteLogErr::InsufficientSpaceForSetup { required_space }) => device_size
                    < required_space,
                _ => false,
            },
    {
        if device_size <= contents_offset {
            return Err(
                InfiniteLogErr::InsufficientSpaceForSetup { required_space: contents_offset + 1 },
            );
        }
        let log_size = device_size - contents_offset;
        let log_header_metadata = PersistentHeaderMetadata { head: 0, tail: 0, log_size };
    ...
    pub exec fn untrusted_start<Perm, PM>(
        wrpm: &mut WriteRestrictedPersistentMemory<Perm, PM>,
        device_size: u64,
        Tracked(perm): Tracked<&Perm>,
    ) -> (result: Result<UntrustedLogImpl, InfiniteLogErr>) where
        ...
        requires
            ...
            header_crc_offset < header_crc_offset + crc_size <= header_head_offset
                < header_tail_offset < header_log_size_offset,
            ...
            ({
                forall|pm_state| #[trigger]
                    perm.check_permission(pm_state) <==> Self::recover(pm_state) == Self::recover(
                        old(wrpm)@,
                    )
            }),
        ensures
            ...
            },
    {
        ...
        let header_bytes = pm.read(
            header_pos + header_head_offset,
            header_size - header_head_offset,
        );
        let ghost header_addrs = Seq::<int>::new(
            (header_size - header_head_offset) as nat,
            |i: int| i + header_pos + header_head_offset,
        );
        let header = if u64_from_le_bytes(bytes_crc(&header_bytes).as_slice()) == u64_from_le_bytes(
            crc_bytes.as_slice(),
        ) {
            proof {
                lemma_auto_spec_u64_to_from_le_bytes();
                lemma_u64_bytes_eq(
                    spec_u64_from_le_bytes(spec_crc_bytes(header_bytes@)),
                    spec_u64_from_le_bytes(crc_bytes@),
                );
    ...
    pub exec fn untrusted_append<Perm, PM>(
        ...
        Tracked(perm): Tracked<&Perm>,
    ) -> (result: Result<u64, InfiniteLogErr>) where
        Perm: CheckPermission<Seq<u8>>,
        PM: PersistentMemory,
            ...
            }),
    {
        ...
        if self.tail > u64::MAX - append_size {
            Err(
                InfiniteLogErr::InsufficientSpaceForAppend {
                    available_space: u64::MAX - self.tail,
                },
            )
        } else if append_size >= self.log_size - (self.tail - self.head) {
            Err(
                InfiniteLogErr::InsufficientSpaceForAppend {
                    available_space: self.log_size - 1 - (self.tail - self.head),
                },
            )
        } else {
            let mut header_metadata = PersistentHeaderMetadata {
                head: self.head,
                tail: self.tail,
            ...
            if physical_head <= physical_tail {
                ...
                if physical_tail + append_size >= physical_head {
                    return Err(
                        InfiniteLogErr::InsufficientSpaceForAppend {
                            available_space: physical_head - physical_tail,
                        },
                    );
                }
                // no wrap
            ...
            proof {
                lemma_header_crc_correct(new_header_bytes@, new_crc_bytes@, old_metadata_bytes);
            }
    ...
    exec fn append_no_wrap<Perm, PM>(
        ...
        requires
            ...
            ({
                let physical_tail = spec_addr_logical_to_physical(
                    old_header.tail as int,
                    old_header.log_size as int,
                );
                physical_tail + bytes_to_append@.len() < old_header.log_size + contents_offset
            }),
            ({
                let physical_head = spec_addr_logical_to_physical(
                    old_header.head as int,
                    old_header.log_size as int,
                );
                let physical_tail = spec_addr_logical_to_physical(
                    old_header.tail as int,
                ...
                &&& physical_head <= physical_tail ==> physical_tail + bytes_to_append@.len()
                    < contents_end
                &&& physical_tail < physical_head ==> physical_tail <= physical_tail
                    + bytes_to_append@.len() < physical_head
            }),
        ensures
            self.inv(wrpm),
            wrpm.constants() == old(wrpm).constants(),
            ...
            ({
                ...
                &&& new_data.subrange(
                    physical_tail - contents_offset,
                    physical_tail - contents_offset + bytes_to_append@.len() as int,
                ) =~= bytes_to_append@
                &&& new_data.subrange(0, physical_tail - contents_offset) =~= old_data.subrange(
                    0,
                    physical_tail - contents_offset,
                )
                &&& new_data.subrange(
                    physical_tail - contents_offset + bytes_to_append@.len(),
                    new_data.len() as int,
                ) =~= old_data.subrange(
                    physical_tail - contents_offset + bytes_to_append@.len(),
                    old_data.len() as int,
            ...
            }),
    {
        ...
        proof {
            lemma_data_write_is_safe(wrpm@, bytes_to_append@, physical_tail as int, perm);
        }
        wrpm.write(physical_tail, bytes_to_append.as_slice(), Tracked(perm));
        proof {
            assert(wrpm@.subrange(0, physical_tail as int) =~= old(wrpm)@.subrange(
                0,
                physical_tail as int,
            ));
            lemma_subrange_equality_implies_subsubrange_equality(
    ...
    pub exec fn append_wrap<Perm, PM>(
        ...
        requires
            ...
            ({
                ...
                &&& physical_head <= physical_tail
                &&& bytes_to_append@.len() <= old_header.log_size - (old_header.tail
                    - old_header.head)
            }),
        ensures
            self.inv(wrpm),
            ...
            ({
                ...
                &&& new_data.subrange(0, len2 as int) =~= bytes_to_append@.subrange(
                    len1 as int,
                    bytes_to_append@.len() as int,
                )
                &&& new_data.subrange(len2 as int, physical_tail - contents_offset)
                    =~= old_data.subrange(len2 as int, physical_tail - contents_offset)
                &&& bytes_to_append@ =~= new_data.subrange(
                    physical_tail - contents_offset,
                    contents_end - contents_offset,
                ) + new_data.subrange(0, len2 as int)
            }),
    {
        ...
        proof {
            lemma_data_write_is_safe(wrpm@, bytes1@, physical_tail as int, perm);
        }
        wrpm.write(physical_tail, bytes1, Tracked(perm));
        proof {
            lemma_data_write_is_safe(wrpm@, bytes2@, contents_offset as int, perm);
        }
        wrpm.write(contents_offset, bytes2, Tracked(perm));
        proof {
            assert(wrpm@.subrange(0, contents_offset as int) =~= old(wrpm)@.subrange(
                0,
                contents_offset as int,
            ));
            lemma_subrange_equality_implies_subsubrange_equality(
    ...
    pub exec fn untrusted_advance_head<Perm, PM>(
        ...
        new_head: u64,
        Tracked(perm): Tracked<&Perm>,
    ) -> (result: Result<(), InfiniteLogErr>) where
        Perm: CheckPermission<Seq<u8>>,
        PM: PersistentMemory,
            ...
            ({
                ...
                match (result, old_log_state, new_log_state) {
                    (Ok(_), Some(old_log_state), Some(new_log_state)) => {
                        &&& old_log_state.head <= new_head <= old_log_state.head
                            + old_log_state.log.len()
            ...
            }),
    {
        ...
        if new_head < live_header.metadata.head {
            ...
            return Err(
                InfiniteLogErr::CantAdvanceHeadPositionBeforeHead {
                    head: live_header.metadata.head,
                },
            );
        }
        if new_head > live_header.metadata.tail {
            assert(self.header_crc == old(self).header_crc);
            return Err(
                InfiniteLogErr::CantAdvanceHeadPositionBeyondTail {
                    tail: live_header.metadata.tail,
                },
            );
        }
        ...
        proof {
            ...
            assert(new_pm.subrange(incorruptible_bool_pos as int, incorruptible_bool_pos + 8)
                =~= new_ib_bytes@);
            let new_header = spec_bytes_to_header(new_header_bytes@);
            let (ib, headers, data) = pm_to_views(new_pm);
            let header_pos = if new_ib == cdb0_val {
            ...
            assert(new_pm.subrange(header_pos as int, header_pos + header_size)
                =~= new_header_bytes@);
            lemma_header_match(new_pm, header_pos as int, new_header);
            lemma_header_correct(new_pm, new_header_bytes@, header_pos as int);
    ...
    pub exec fn untrusted_read<Perm, PM>(
        ...
        pos: u64,
        len: u64,
    ) -> (result: Result<Vec<u8>, InfiniteLogErr>) where
        Perm: CheckPermission<Seq<u8>>,
        PM: PersistentMemory,
            ...
            }),
    {
        ...
        if pos < self.head {
            ...
            if physical_head == physical_tail {
                assert(Seq::<u8>::empty() =~= log.log.subrange(
                    pos - log.head,
                    pos + len - log.head,
                ));
                let buf = Vec::new();
                let ghost addrs = Seq::<int>::empty();
                assert(if wrpm.constants().impervious_to_corruption {
                    buf@ == true_bytes
                } else {
                    maybe_corrupted(buf@, true_bytes, addrs)
                });
                Ok(buf)
                ...
                let ghost r1_addrs = Seq::<int>::new(
                    r1_len as nat,
                    |i: int| i + physical_pos as int,
                );
                let ghost r2_addrs = Seq::<int>::new(
                    r2_len as nat,
                    |i: int| i + contents_offset as int,
                );
                let ghost addrs: Seq<int> = r1_addrs.add(r2_addrs);
                r1.append(&mut r2);
                assert(pm@.subrange(physical_pos as int, physical_pos + r1_len) + pm@.subrange(
                    contents_offset as int,
                    contents_offset + r2_len,
                ) =~= log.log.subrange(pos - log.head, pos + len - log.head));
                assert(if wrpm.constants().impervious_to_corruption {
                    r1@ == true_bytes
                } else {
                    maybe_corrupted(r1@, true_bytes, addrs)
                });
                Ok(r1)
            } else {
                assert(pm@.subrange(physical_pos as int, physical_pos + len) =~= log.log.subrange(
                    pos - log.head,
                    pos + len - log.head,
                ));
    ...
    pub exec fn untrusted_get_head_and_tail<Perm, PM>(
        &self,
        wrpm: &WriteRestrictedPersistentMemory<Perm, PM>,
    ) -> (result: Result<(u64, u64, u64), InfiniteLogErr>) where
        Perm: CheckPermission<Seq<u8>>,
        PM: PersistentMemory,
        ...
        ensures
            match result {
                Ok((result_head, result_tail, result_capacity)) => match Self::recover(
                    wrpm@,
                ).unwrap() {
...
impl TrustedPermission {
    proof fn new(
        ...
        ensures
            forall|s| #[trigger]
                perm.check_permission(s) <==> crate::sccf::is_state_allowable(
                    cur,
                    s,
                    recovery_view(),
                    next,
    ...
    {
        Self {
            is_state_allowable: |s| crate::sccf::is_state_allowable(cur, s, recovery_view(), next),
        }
    }
}
...
impl<PM: PersistentMemory> InfiniteLogImpl<PM> {
    ...
    pub exec fn setup(pm: &mut PM, device_size: u64) -> (result: Result<u64, InfiniteLogErr>)
        ...
        ensures
            ...
            pm@.len() == device_size,
            match result {
                Ok(log_capacity) => recovery_view()(pm@) == Some(
                    AbstractInfiniteLogState::initialize(log_capacity as int),
                ),
                Err(InfiniteLogErr::InsufficientSpaceForSetup { required_space }) => device_size
                    < required_space,
                _ => false,
            ...
            },
    {
        ...
        // appended.
        let tracked perm = TrustedPermission::new(
            self.wrpm@,
...
#[verifier(external_body)]
pub proof fn axiom_bytes_uncorrupted(
    x_c: Seq<u8>,
    x: Seq<u8>,
    x_addrs: Seq<int>,
    y_c: Seq<u8>,
...
pub open spec fn update_byte_to_reflect_write(
    addr: int,
    prewrite_byte: u8,
    write_addr: int,
    write_bytes: Seq<u8>,
...
pub open spec(checked) fn update_contents_to_reflect_write(
    prewrite_contents: Seq<u8>,
    write_addr: int,
    write_bytes: Seq<u8>,
) -> Seq<u8>
    recommends
...
{
    Seq::<u8>::new(
        prewrite_contents.len(),
        |addr| update_byte_to_reflect_write(addr, prewrite_contents[addr], write_addr, write_bytes),
    )
}
...
pub open spec fn update_byte_to_reflect_partially_flushed_write(
    addr: int,
    prewrite_byte: u8,
    write_addr: int,
    write_bytes: Seq<u8>,
...
/// chunk IDs that have been flushed since the initiation of
/// the write is `chunks_flushed`.
pub open spec(checked) fn update_contents_to_reflect_partially_flushed_write(
    contents: Seq<u8>,
    write_addr: int,
    write_bytes: Seq<u8>,
    chunks_flushed: Set<int>,
        ...
        write_addr + write_bytes.len() <= contents.len(),
{
    Seq::<u8>::new(
        contents.len(),
        |addr|
            update_byte_to_reflect_partially_flushed_write(
                addr,
                contents[addr],
                write_addr,
                write_bytes,
...
pub struct WriteRestrictedPersistentMemory<Perm, PM> where
    Perm: CheckPermission<Seq<u8>>,
    PM: PersistentMemory,
 {
    pm: PM,
...
impl<Perm, PM> WriteRestrictedPersistentMemory<Perm, PM> where
    Perm: CheckPermission<Seq<u8>>,
    PM: PersistentMemory,
 {
